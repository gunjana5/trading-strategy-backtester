...
def _cost_risk_from_body(body: dict) -> dict:
    # costs can sit top-level or inside params - accept both
    # 0 on the halt/stop sliders means off - engine wants None not 0.0
...
def _apply_strategy(df, strategy_key, params):
    # signals once - then backtest with/without costs on the same df
    params = params or {}
    validation = None
