...
def run(df, walk_forward: bool = True, n_folds: int = 3):
    """returns (df with signal, validation meta). walk_forward default matches the ui."""
    # app.py only talks to this; expanding folds + features are in walk_forward
    if walk_forward:
        return run_walk_forward(df, n_folds=n_folds)
    # optional: one chronological 70/30 hold-out
